...
// ✅ Worker correto — .min.js compatível com todos os ambientes
pdfjs.GlobalWorkerOptions.workerSrc = `//unpkg.com/pdfjs-dist@${pdfjs.version}/build/pdf.worker.min.js`;

interface Props {
  pdfUrl: string;       // receberá "/api/pdf/[chapterId]"
  comicId: string;
  chapterNum: number;
  chapterTitle?: string;
...
export default function PDFLeitorPage({
  pdfUrl,
  comicId,
  chapterNum,
  chapterTitle,
  ...
  const goNext = useCallback(() => {
    if (current < numPages) setCurrent((c) => c + 1);
  }, [current, numPages]);
          ...
          <div className="flex items-center justify-between gap-2">
            ...
            {nextChapterId ? (
              <Link
                ...
                className="flex items-center gap-1.5 text-white/60 hover:text-white transition-colors text-xs flex-shrink-0"
              >
                <span className="hidden sm:inline">Próx. capítulo</span>
                <ArrowRight size={14} />
              </Link>
            ) : (
