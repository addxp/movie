  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    if (error) {
      ...
      const msg: Record<string, string> = {
        ...
        "Database error saving new user": "Erro interno. Tente novamente em instantes.",
      };

      const translated = Object.entries(msg).find(([key]) =>
        error.message.toLowerCase().includes(key.toLowerCase())
      );

      setError(translated ? translated[1] : `Erro: ${error.message}`);
      setLoading(false);
      return;
    }
  ...
  if (success) {
    return (
      <div className="bg-black/80 backdrop-blur-md rounded-2xl p-8 border border-white/10 shadow-2xl text-center">
        ...
        <Link
          href="/auth/login"
          className="inline-block bg-[var(--color-red)] hover:bg-red-600 text-white font-semibold py-3 px-8 rounded-lg transition-all"
        >
          Ir para o Login
        </Link>
      </div>
  ...
  return (
    <div className="bg-black/80 backdrop-blur-md rounded-2xl p-8 border border-white/10 shadow-2xl">
      ...
      <form onSubmit={handleRegister} className="space-y-4">
        <div className="relative">
          ...
          <input
            ...
            required
            className="input-glow w-full bg-white/5 border border-white/10 rounded-lg pl-10 pr-4 py-3 text-white placeholder:text-[var(--color-muted)] transition-all hover:border-white/20"
          />
        </div>
        ...
        <div className="relative">
          ...
          <input
            ...
            required
            className="input-glow w-full bg-white/5 border border-white/10 rounded-lg pl-10 pr-10 py-3 text-white placeholder:text-[var(--color-muted)] transition-all hover:border-white/20"
          />
          <button
            type="button"
        ...
        <button
          type="submit"
          disabled={loading}
          className="w-full bg-[var(--color-red)] hover:bg-red-600 text-white font-semibold py-3 rounded-lg transition-all flex items-center justify-center gap-2 disabled:opacity-60"
        >
          {loading ? (
      ...
      <p className="mt-6 text-center text-[var(--color-muted)] text-sm">
        Já tem uma conta?{" "}
        <Link href="/auth/login" className="text-white hover:text-[var(--color-red)] transition-colors font-medium">
          Entrar
        </Link>
      </p>
    </div>
