...
import { getProfileById, getUserReviews, getProfileStats } from "@/lib/profiles";
import { getUserFavorites } from "@/lib/movies";
import Navbar from "@/components/layout/Navbar";
import ProfileEditor from "@/components/profile/ProfileEditor";
import StatsRow from "@/components/profile/StatsRow";
import ReviewComposer from "@/components/profile/ReviewComposer";
...
export default async function MyProfilePage() {
  ...
  const [profile, reviews, favorites, stats] = await Promise.all([
      ...
      <div style={{ maxWidth: "920px", margin: "0 auto", padding: "40px 0 80px" }}>
        ...
        <div style={{ padding: "0 20px" }}>
          <section style={{ marginTop: "40px" }}>
            ...
            {favorites.length === 0 ? (
              <p style={emptyText}>Você ainda não favoritou nenhum título.</p>
            ) : (
          ...
          <section style={{ marginTop: "40px" }}>
            <h2 style={sectionTitle}>Avaliar um filme</h2>
            <ReviewComposer userId={user.id} />
          </section>

          <section style={{ marginTop: "40px" }}>
            <h2 style={sectionTitle}>Suas Avaliações</h2>
            {reviews.length === 0 ? (
              <p style={emptyText}>Você ainda não avaliou nenhum filme.</p>
            ) : (
