...
import { getProfileByUsername, getFollowing } from "@/lib/profiles";
    ...
    <div style={{ minHeight: "100vh", background: "var(--bg)" }}>
      ...
      <div style={{ maxWidth: "640px", margin: "0 auto", padding: "40px clamp(16px, 5vw, 40px) 80px" }}>
        <h1 style={{ fontFamily: "var(--font-display)", fontWeight: 800, fontSize: "22px", color: "var(--text)", marginBottom: "4px" }}>
          Seguindo @{profile.username}
        </h1>
        <p style={{ color: "var(--text-3)", fontSize: "13px", marginBottom: "28px" }}>{following.length} {following.length === 1 ? "pessoa" : "pessoas"}</p>

        {following.length === 0 ? (
          <p style={{ color: "var(--text-3)", fontSize: "13px" }}>Ainda não segue ninguém.</p>
        ) : (
