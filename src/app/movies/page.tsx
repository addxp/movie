...
export default async function FilmesPage({ searchParams }: { searchParams: Promise<{ page?: string }> }) {
  ...
  const { data: filmes, count } = await supabase
    .from("movies")
    .select("*", { count: "exact" })
    .eq("type", "movie")
      ...
      <div className="pt-24 px-8 lg:px-16 pb-16">
        ...
        {!filmes || filmes.length === 0 ? (
          <div className="text-center py-20">
            ...
            <p className="text-[#555]">Nenhum filme cadastrado ainda.</p>
          </div>
        ) : (
          <>
            <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-7 xl:grid-cols-8 gap-3">
              {filmes.map((m) => (
                <Link key={m.id} href={"/movie/" + m.id} className="group block">
                  ...
                  <h3 className="text-white/90 text-xs font-medium line-clamp-1 group-hover:text-[var(--color-red)] transition-colors">{m.title}</h3>
