...
export default async function MoviePage({ params }: Props) {
  ...
  return (
    <div className="zone-vault min-h-screen bg-[var(--bg)]">
      <Navbar user={user} />
      ...
      <div className="fixed inset-0 z-0 pointer-events-none">
        {bgImage && (
          <>
            ...
            <div className="absolute inset-0 bg-gradient-to-b from-black/60 via-[var(--bg)]/80 to-[var(--bg)]" />
          </>
        )}
      </div>
      ...
      <div className="relative z-10 pt-24 pb-16 max-w-6xl mx-auto px-6 lg:px-8">
          ...
          <div className="relative w-full h-64 md:h-80 rounded-2xl overflow-hidden mb-8 shadow-2xl">
            ...
            <div className="absolute bottom-6 left-6 right-6">
              <div className="flex items-center gap-3 mb-2">
                <span className="bg-[var(--red)] text-white text-xs font-bold tracking-widest uppercase px-3 py-1 rounded">
                  {movie.category}
                </span>
                {isSeries && (
        ...
        <div className="grid lg:grid-cols-[260px_1fr] gap-8">
          ...
          <div>
            ...
            <div className="mb-8 border border-[var(--red-border)] rounded-md p-4 bg-[var(--bg-3)]/60 relative">
              <span
                className="vault-stamp absolute -top-3 left-4 bg-[var(--bg)] px-2"
                style={{ fontFamily: "var(--font-mono)" }}
              >
                Arquivo confidencial
              </span>
              <h2 className="text-[var(--red-2)] text-xs font-bold tracking-widest uppercase mb-2" style={{ fontFamily: "var(--font-mono)" }}>
                Sinopse
              </h2>
              ...
              {isSeries ? (
                <div className="space-y-6">
                  ...
                  {Object.keys(seasons).length === 0 ? (
                    <p className="text-[var(--text-3)]">Nenhum episódio cadastrado ainda.</p>
                  ) : (
                    Object.entries(seasons).map(([season, eps]) => (
                      <div key={season} className="bg-black/30 backdrop-blur-sm rounded-xl p-4 border border-white/5">
                        <h3 className="text-white font-semibold mb-3 flex items-center gap-2">
                          <span className="bg-[var(--red)] text-white text-xs px-2 py-0.5 rounded">T{season}</span>
                          Temporada {season}
                        </h3>
                            ...
                            >
                              {ep.thumbnail ? (
                                ...
                                <div className="w-24 h-14 flex-shrink-0 rounded bg-white/5 flex items-center justify-center">
                                  <span className="text-[var(--text-3)] text-xs">E{ep.episode}</span>
                                </div>
                              )}
                              <div className="flex-1">
                                <p className="text-white text-sm font-medium group-hover:text-[var(--red)] transition-colors">
                                  {ep.episode}. {ep.title}
                                </p>
                                {ep.description && (
                                  <p className="text-[var(--text-3)] text-xs line-clamp-1 mt-0.5">{ep.description}</p>
                                )}
                              </div>
                              {ep.duration && (
                                <span className="text-[var(--text-3)] text-xs flex-shrink-0">{ep.duration}m</span>
                              )}
                            </Link>
                          ))}
                        </div>
                ...
                movie.video_url ? (
                  ...
                  <div className="w-full aspect-video bg-black/30 rounded-xl flex items-center justify-center border border-white/5">
                    <p className="text-[var(--text-3)]">Vídeo não disponível</p>
                  </div>
                )
              )}
            </div>
