...
import { getProfileByUsername, getUserReviews, getProfileStats, isFollowing } from "@/lib/profiles";
...
export default async function PublicProfilePage({ params }: { params: Promise<{ username: string }> }) {
  ...
  const [reviews, favorites, stats, following] = await Promise.all([
    getUserReviews(profile.id),
    getUserFavorites(profile.id),
    getProfileStats(profile.id),
    user ? isFollowing(user.id, profile.id) : Promise.resolve(false),
  ]);
  ...
  return (
    <div style={{ minHeight: "100vh", background: "var(--bg)" }}>
      ...
      <div style={{ maxWidth: "920px", margin: "0 auto", padding: "40px 0 80px" }}>
        {/* Capa */}
        <div style={{
          height: "180px", borderRadius: "var(--radius)", overflow: "hidden", margin: "0 20px",
          ...
          border: "1px solid var(--border-2)", width: "calc(100% - 40px)",
        }} />
        ...
        <div style={{ display: "flex", alignItems: "flex-end", justifyContent: "space-between", marginTop: "-46px", padding: "0 20px" }}>
          <div style={{ display: "flex", alignItems: "flex-end", gap: "16px" }}>
            ...
            <div style={{ paddingBottom: "6px" }}>
              ...
              <p style={{ color: "var(--text-3)", fontSize: "13px" }}>@{profile.username}</p>
            </div>
          </div>
          <FollowButton viewerId={user?.id ?? null} targetId={profile.id} initialFollowing={following} />
        </div>
